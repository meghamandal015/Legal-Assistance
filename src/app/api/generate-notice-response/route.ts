    ...
    messages: [
      {
        ...
        content: `Analyze this legal notice provided and generate an appropriate, properly formatted reply for the notice: 
          ...
          In addition, consider the following questions and answers while generating the reply:${QnAs}
...
export async function POST(request: Request) {
    try {
        ...
        const submittedData = existingData?.submitted_data || "";
        const numOfWords = existingData?.num_of_words || "";

        const response = await claudeResponse(submittedData, noticeText, currentDate, numOfWords)
